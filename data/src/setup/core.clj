...
(ns setup.core
    ...
    (let [json-string-mapper (map json/parse-string)
      ...
      (with-open [kafkas (io/reader "kafka-json.txt")]
        (transduce (comp json-string-mapper campaign-buckets-mapper)
                   ...
                   (line-seq kafkas)))))
  (defn parse-long [s]
    (if (some? s) (Long. (re-find  #"\d+" s )) (long 0)))
  (defn seenCalculation [redis-host partitions campaign time-bucket]
    (apply +
      (redis/with-server {:host redis-host}
        (for [iterator (range 0 partitions)]
          (let [winUUID (redis/hget (redis/hget iterator campaign) time-bucket)]
            (parse-long (redis/hget winUUID "seen_count")))))))
  (defn updatedCalculation [redis-host partitions campaign time-bucket]
    (apply max
      (redis/with-server {:host redis-host}
        (for [iterator (range 0 partitions)]
          (let [winUUID (redis/hget (redis/hget iterator campaign) time-bucket)]
            (parse-long (redis/hget winUUID "time_updated")))))))
  (defn lastEventCalculation [redis-host partitions campaign time-bucket]
    (apply max
      (redis/with-server {:host redis-host}
        (for [iterator (range 0 partitions)]
          (let [winUUID (redis/hget (redis/hget iterator campaign) time-bucket)]
            (parse-long (redis/hget winUUID "last_event_time")))))))
  (defn sumAvgCalculation [redis-host partitions campaign time-bucket]
    (apply +
      (redis/with-server {:host redis-host}
        (for [iterator (range 0 partitions)]
          (let [winUUID (redis/hget (redis/hget iterator campaign) time-bucket)]
            (* (parse-long (redis/hget winUUID "avg_pre_window_latency")) (parse-long (redis/hget winUUID "seen_count"))))))))
  (defn cp-prosprocessing [redis-host partitions]
  (letfn [(data-writer [[windowListUUID campaign time-bucket newWindowUUID seen_count time_updated avg_pre_window_latency last_event_time]]
    ;(println {:windowListUUID windowListUUID :campaign campaign :time-bucket time-bucket :newWindowUUID newWindowUUID :seen_count seen_count :time_updated time_updated :avg_pre_window_latency avg_pre_window_latency :last_event_time last_event_time})
    (redis/hset campaign "windows" windowListUUID)
    (redis/hset campaign time-bucket newWindowUUID)
    (redis/lpush windowListUUID time-bucket)
    (redis/hset newWindowUUID "seen_count" seen_count)
    (redis/hset newWindowUUID "time_updated" time_updated)
    (redis/hset newWindowUUID "avg_pre_window_latency" avg_pre_window_latency)
    (redis/hset newWindowUUID "last_event_time" last_event_time)
    )]
    (redis/with-server {:host redis-host}
          ...
          (map data-writer
            (apply concat
              (for [campaign campaigns]
                (let [nodeWindowUUID (redis/hget "0" campaign)
                      windowListUUID (.toString (java.util.UUID/randomUUID))
                      time-buckets (redis/hkeys nodeWindowUUID)]
                  ;;(redis/hset campaign "windows" windowListUUID)
                  (for [time-bucket time-buckets]
                    (let [windowUUID (redis/hget nodeWindowUUID time-bucket)
                      seen_count (seenCalculation redis-host partitions campaign time-bucket)
                      time_updated (updatedCalculation redis-host partitions campaign time-bucket)
                      sum_avg_pre_window_latency (sumAvgCalculation redis-host partitions campaign time-bucket)
                      last_event_time (lastEventCalculation redis-host partitions campaign time-bucket)
                      newWindowUUID (.toString (java.util.UUID/randomUUID))]
                        [windowListUUID campaign time-bucket newWindowUUID seen_count time_updated (float (/ sum_avg_pre_window_latency seen_count)) last_event_time])))))))))))
  (defn get-stats [redis-host]
    (with-open [seen-file (clojure.java.io/writer "seen.txt")
                ...
                time-bucket-file (clojure.java.io/writer "time-bucket.txt")]
      (letfn [(data-printer [[seen updated pre-window latency time-bucket]]
                (.write seen-file (str seen "\n"))
                (.write updated-file (str updated "\n"))
                (.write pre-window-file (str pre-window "\n"))
                (.write latency-file (str latency "\n"))
                (.write time-bucket-file (str time-bucket "\n")))]
        (redis/with-server {:host redis-host}
          (doall
           (map data-printer
                (apply concat
                           ...
                           (let [windows-key (redis/hget campaign "windows")
                             ...
                             (for [window-time windows]
                               (let [window-key (redis/hget campaign window-time)
                                 ...
                                 [seen (- (Long/parseLong time_updated) (Long/parseLong window-time)) pre-window (- (Long/parseLong time_updated) (Long/parseLong latency)) window-time]))))))))))))
      ...
      (with-open [p (producer {"bootstrap.servers" kafka-hosts
        ...
        (println "Load-Startup-Phase finished! Beginning to send events to Kafka")
            (let [start-time-ns (* 1000000 (System/currentTimeMillis))
              period-ns (long (/ 1000000000 (/ throughput (* cpus 10)) ))
              ...
              str3 (str "\", \"ip_address\": \"1.2.3.4\"}")]
                ...
                (doseq [t times]
                  ...
                  (dotimes [n 10] (send p (record "ad-events"
  ...
  (defn get-conf [confPath]
    (let [conf (yaml/parse-string (slurp confPath))
          ...
          kafka-partitions (get conf :kafka.partitions)]
      (println {:redis-host redis-host :kakfa-brokers kafka-hosts :kafka-partitions kafka-partitions})
      {:redis-host redis-host :kakfa-brokers kafka-hosts :kafka-partitions kafka-partitions}))
  ...
  (def cli-options
    [["-s" "--setup" "Set up for catchup-simulation-mode (or re-setup if the .txt files exist)"]
     ...
     ["-o" "--cp-postprocessing" "Read through redis and transfer windows per partition to window per Run."]
  ...
  (defn -main
    ...
    (let [{:keys [options arguments errors summary]} (cli/parse-opts args cli-options)
      ...
      (cond
        ...
        (:cp-postprocessing options)            (cp-prosprocessing redis-host kafka-partitions)
